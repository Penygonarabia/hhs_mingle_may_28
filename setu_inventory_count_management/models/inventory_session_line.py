...
class InventoryCountSessionLine(models.Model):
    ...
    # not_found_serial_number_ids = fields.Many2many('stock.production.lot', 'session_not_found_stock_lot_rel', 'session_line_id',
    #                                                'lot_id', string="Not Founded Serial Numbers")
    ...
    # theoretical_value = fields.Monetary('Theoretical Value', compute='_compute_value',store=True, groups='stock.group_stock_manager')
    # currency_id = fields.Many2one('res.currency', compute='_compute_value',store=True, groups='stock.group_stock_manager')
    # counted_value = fields.Monetary('Counted Value',store=True)
    #
    #
    
    @api.onchange('product_id')
    def _oncange_pdt(self):
        for rec in self:
            return {'domain':{'product_id':[('stock_quant_ids.location_id', '=',self.session_id.location_id.display_name)]}}
        
    # domain="[('stock_quant_ids.location_id', 'ilike','MDU/Stock')]"
    ...
    #     for quant in self:
    #         quant.currency_id = quant.session_id.company_id.currency_id
    #
    ...
    #             quantity=0
    #             for qua in quants:
    #                 # if qua.product_id.cost_method == "average":
    #                 #     quantity = quant.product_id.with_company(quant.session_id.company_id).quantity_svl
    #                 #     print("/////////////////////",val)
    #                 #     average_cost = quant.product_id.with_company(quant.session_id.company_id).value_svl / quantity
    #                 #     quant.value = quantity * average_cost
    #                 if qua.product_id.cost_method == 'fifo':
    #                     quantity = qua.product_id.with_company(quant.session_id.company_id).quantity_svl
    #                     if float_is_zero(quantity, precision_rounding=qua.product_id.uom_id.rounding):
    #                         quant.theoretical_value = 0.0
    #                         continue
    #                     average_cost = qua.product_id.with_company(quant.session_id.company_id).value_svl / quantity
    #                     quant.theoretical_value = qua.quantity * average_cost
    #                     # quant.counted_value = quant.scanned_qty * quant.theoretical_value / quant.theoretical_qty
    #                 else:
    #                     quant.theoretical_value = qua.quantity * qua.product_id.with_company(quant.session_id.company_id).standard_price
    #                     # quant.counted_value = quant.scanned_qty * quant.theoretical_value / quant.theoretical_qty
    #
    #
    #
    # @api.onchange('scanned_qty')
    # def _compute_counted_value(self):
    #     for quant in self:
    #
    #         quant.counted_value = quant.scanned_qty * quant.theoretical_value / quant.theoretical_qty
    #
    ...
    def _onchange_product_id(self):
        ...
        for rec in self:
            ...
            if rec.product_id:
                location_id = rec.session_id.location_id
                bar_code = rec.product_id.barcode
                uom = rec.product_id.uom_id
                cost = rec.product_id.standard_price
                rec.session_id.session_line_ids.location_id = location_id
                rec.session_id.session_line_ids.barcode = bar_code
                rec.session_id.session_line_ids.uom_id = uom
                rec.session_id.session_line_ids.unit_cost = cost
              
        
        # if self.product_id:
        #     self.session_id.session_line_ids.filtered(lambda line: line.product_id == self.product_id).update({
        #     'location_id': self.session_id.location_id,
            ...
            if self.product_id.tracking == 'lot' and self.product_id and self.location_id and self.lot_id and not self.session_id.session_id:
                ...
                #         'product_id').ids:  # and x.location_id == self.current_scanning_location_id
                    ...
                    if same_found:
                        ...
                        # same_found = self.env['stock.production.lot'].sudo().browse(same_found)
