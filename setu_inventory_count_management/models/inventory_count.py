...
class StockInvCount(models.Model):
    ...
    inventory_count_date = fields.Date(
        ...
        string="Inventory Count Date",tracking = True
    )
    ...
    def write(self, vals):
        res = super(StockInvCount, self).write(vals)
        return res
    ...
    def onchange_warehouse_id(self):
        if self.warehouse_id:
            domain = [('usage', '=', 'internal'),('location_id.warehouse_id', '=', self.warehouse_id.id)]
            location = self.env['stock.location'].search(domain)
            return { 'value':{'location_id':location.id}
                
                }
            # return {'domain': {'location_id': [('usage', '=', 'internal'), ('id', 'child_of', self.warehouse_id.view_location_id.id)]}}
        else:
