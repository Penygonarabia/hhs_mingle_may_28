...
class InventoryCountSession(models.Model):
    ...
    update_product = fields.Boolean('Update Session line')
    theoretical_quantity = fields.Boolean('Theoretical Quantity')
    # @api.model
    # def get_views(self):
    ...
    def _get_location_lines_values(self):
    ...
    def pdf_report(self):
        # self.write({'state': "In Progress" or "Done"})
        return self.env.ref('setu_inventory_count_management.report_inventory_count_session_inprogress').report_action(self)

    @api.onchange('update_product')
    def _onchange_product_update(self):
        for rec in self:
            if not rec.update_product:
                rec.session_line_ids = [(5,0,0)]
    ...
    def onchange_origin_location(self):
        ...
        if self.update_product == True:
            lines = []
            line_model = self.env["setu.inventory.count.session.line"]
            for line_val in self._get_location_lines_values():
                line = line_model.create(line_val)
                lines.append(line)
            # for line in self.session_line_ids:
            #     line._onchange_product_id()
                line._get_theoretical_qty(count_line_exists_already=False, location=False)
                # line._compute_value()
            self.update(
                {"session_line_ids": [(6, 0, [line.id for line in lines])]}
            )
        else:
            lines = []
            self.update(
    ...
    def on_barcode_scanned(self, barcode):
        if self.use_barcode_scanner:
            if self.current_state in ('Start', 'Resume'):
                ...
                # lot = self.env['stock.production.lot'].sudo().search([('name', '=ilike', barcode)], limit=1)
    ...
    def action_open_child_sessions(self):
        ...
        action = \
            self.sudo().env.ref('setu_inventory_count_management.inventory_count_session_act_window').read()[0]
