...
class StockInventoryLine(models.Model):
    ...
    theoretical_qty = fields.Float(
        string="Theoretical QTY",tracking = True
    )
    product_qty = fields.Float(
        string="Counted QTY",tracking = True
    )
    inventory_id = fields.Many2one(
        comodel_name="setu.stock.inventory",
        string="Inventory",tracking = True
    )
    ...
    serial_number_ids = fields.Many2many('stock.lot', 'setu_stock_inventory_line_stock_lot_rel', 'setu_stock_inventory_line_id', 'stock_production_lot_id', string="Serial Numbers")
    ...
    #     domain="[('product_id','=',product_id), ('company_id', '=', company_id)]"
    # )
    # serial_number_ids = fields.Many2many('stock.production.lot', 'setu_stock_inventory_line_stock_lot_rel', 'setu_stock_inventory_line_id', 'stock_production_lot_id', string="Serial Numbers")
    # not_found_serial_number_ids = fields.Many2many('stock.production.lot', 'setu_not_found_line_stock_lot_rel', 'inventory_line_id', 'lot_id', string="Missing Serial Numbers")
    #
    ...
    new_serial_number_ids = fields.Many2many('stock.production.lot', 'setu_new_serial_stock_lot_rel', 'inventory_line_id', 'lot_id',
...
class StockInventory(models.Model):
    ...
    date = fields.Date(
        string="Inventory Date",tracking=True
    )
    ...
    @api.model
    def create(self, vals):
        record = super(StockInventory, self).create(vals)
        record.message_post(body="Record created by %s" % self.env.user.name)
        return record

    def write(self, vals):
        res = super(StockInventory, self).write(vals)
        self.message_post(body="Record updated by %s" % self.env.user.name)
        return res
