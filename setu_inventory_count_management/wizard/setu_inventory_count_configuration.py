...
class SetuInventoryCountConfiguration(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        install_setu_inventory_count_extended = self.env['ir.config_parameter'].sudo().get_param(
            'setu_inventory_count_management.extended_ic_module_in_registry')
    ...
    def execute(self):
        install_setu_inventory_count = self.env['ir.config_parameter'].sudo().get_param('setu_inventory_count_management.extended_ic_module_in_registry')
        self.unzip_and_install_extended_module(True, install_setu_inventory_count)
        self.env['ir.config_parameter'].set_param('setu_inventory_count_management.extended_ic_module_in_registry', True)
        return {
    ...
    def unzip_and_install_extended_module(self, state, install_setu_inventory_count):
        try:
            ...
            target_dir = __file__.split('/setu_inventory_count_management/wizard/setu_inventory_count_configuration.py')[0]
            ...
            if state:
                if extended_inventory_count and extended_inventory_count.state != 'installed':
                    ...
                    self.env['ir.module.module'].sudo().update_list_setu_inventory_count_management()
            return True
