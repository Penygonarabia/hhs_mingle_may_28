...
class TrialBalanceXslx(models.AbstractModel):
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
            ...
            if grouped_by:
                for grouped_item in trial_balance_grouped:
                    ...
                    for balance in grouped_item["account_data"]:
                        ...
                        # Accumulate totals
                        total_initial_balance += balance.get("initial_balance", 0.0)
                        total_debit += balance.get("debit", 0.0)
                        total_credit += balance.get("credit", 0.0)
        ...
        # ========================
        report_data["row_pos"] += 1  # Move to next row
        ...
        total_row = {
            ...
            "debit": total_debit,
            "credit": total_credit,
            "balance": total_initial_balance + total_debit - total_credit,  # Example calculation
            "currency_id": False,
            "ending_balance": total_initial_balance + total_debit - total_credit,
        }

        self.write_account_footer(total_row, "Grand Total", report_data)
