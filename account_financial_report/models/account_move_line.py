...
class AccountMoveLine(models.Model):
    ...
    def _compute_analytic_account_ids(self):
        ...
        for record in self.filtered("analytic_distribution"):
            try:
                # Process each key in analytic_distribution
                for key in record.analytic_distribution:
                    # Split key and attempt to convert each part to an integer
                    for account_id_str in key.split(","):
                        try:
                            account_id = int(account_id_str)
                            batch_by_analytic_account[account_id] += record
                        except ValueError:
                            _logger.warning(
                                f"Invalid analytic account ID '{account_id_str}' in "
                                f"analytic_distribution for record {record.id}. Skipping."
                            )
                            continue
            except Exception as e:
                _logger.error(
                    f"Error processing analytic_distribution for record {record.id}: {str(e)}"
                )
                continue

        # Fetch existing analytic accounts
        existing_account_ids = set(
            self.env["account.analytic.account"]
            .browse(batch_by_analytic_account.keys())
            .exists()
            .ids
        )

        # Update analytic_account_ids for each record
        for record in self:
            # Clear existing links
            record.analytic_account_ids = [Command.clear()]
            # Link valid analytic accounts
            for account_id, records in batch_by_analytic_account.items():
                if account_id in existing_account_ids and record in records:
                    record.analytic_account_ids = [(Command.link(account_id))]
