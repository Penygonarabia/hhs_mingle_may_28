...
class EmployeeSelfService(models.Model):
    ...
    employee = fields.Many2one('hr.employee', string='Employee',tracking=True,default=_get_employee_id)
   # emp_manager = fields.Char(string='Department Manager',compute='compute_emp_manager')
    ...
    employee_end_date = fields.Date(string='Last Working Date',tracking=True)
    ...
    template = fields.Html(tracking=True)
    template_value = fields.Char(tracking=True)
    ...
    #             #Newly Added
    #             if str(self.old_employee_id.contract_id.wage) in self.template:
    #                 employee_wage = self.template.replace(str(self.old_employee_id.contract_id.wage),
    #                                                       str(self.employee.contract_id.wage))
    #                 self.template = employee_wage
    #             if str(self.old_employee_id.employee_no) in self.template:
    #                 employee_no = self.template.replace(str(self.old_employee_id.employee_no), str(self.employee.employee_no))
    #                 self.template = employee_no
    #
    #             if str(self.date) in self.template:
    #                 date = self.template.replace(str(self.date))
    #                 self.template = date
    #
    #             if str(self.old_employee_id.birthday) in self.template:
    #                 birthday = self.template.replace(str(self.old_employee_id.birthday), str(self.employee.birthday))
    #                 self.template = birthday
    #
    #             if str(self.old_employee_id.passport_expiry_date) in self.template:
    #                 passport_expiry_date = self.template.replace(str(self.old_employee_id.passport_expiry_date), str(self.employee.passport_expiry_date))
    #                 self.template = passport_expiry_date
    #
    #             if str(self.old_employee_id.iqama_no) in self.template:
    #                 iqama_no = self.template.replace(str(self.old_employee_id.iqama_no), str(self.employee.iqama_no))
    #                 self.template = iqama_no
    #
    #             if str(self.old_employee_id.work_location_id.name) in self.template:
    #                 work_location_id = self.template.replace(str(self.old_employee_id.work_location_id.name), str(self.employee.work_location_id.name))
    #                 self.template = work_location_id
    #
    ...
    #                     self.template = net_salary
    #
    #             # if 'amount_in_word' in self.template:str(self.date)
    ...
    #                     self.template = join_date
    #             if 'employee_wage' in self.template:
    #                 if self.employee.contract_id.wage:
    #                     employee_wage = self.template.replace('employee_wage', str(self.employee.contract_id.wage))
    #                     self.template = employee_wage
    #                 else:
    #                     employee_wage = self.template.replace('employee_wage', '')
    #                     self.template = employee_wage
    #             if 'employee_no' in self.template:
    #                 if self.employee.employee_no:
    #                     employee_no = self.template.replace('employee_no', str(self.employee.employee_no))
    #                     self.template = employee_no
    #                 else:
    #                     employee_no = self.template.replace('employee_no', '')
    #                     self.template = employee_no
    #
    #             if 'date' in self.template:
    #                 if self.date:
    #                     date = self.template.replace('date', str(self.date))
    #                     self.template = date
    #                 else:
    #                     date = self.template.replace('date', '')
    #                     self.template = date
    #
    #             if 'birthday' in self.template:
    #                 if self.employee.birthday:
    #                     birthday = self.template.replace('birthday', str(self.employee.birthday))
    #                     self.template = birthday
    #                 else:
    #                     date = self.template.replace('date', '')
    #                     self.template = date
    #
    #             if 'passport_expiry_date' in self.template:
    #                 if self.employee.passport_expiry_date:
    #                     passport_expiry_date = self.template.replace('passport_expiry_date', str(self.employee.passport_expiry_date))
    #                     self.template = passport_expiry_date
    #                 else:
    #                     passport_expiry_date = self.template.replace('passport_expiry_date', '')
    #                     self.template = passport_expiry_date
    #
    #             if 'iqama_no' in self.template:
    #                 if self.employee.iqama_no:
    #                     iqama_no = self.template.replace('iqama_no', str(self.employee.iqama_no))
    #                     self.template = iqama_no
    #                 else:
    #                     iqama_no = self.template.replace('iqama_no', '')
    #                     self.template = iqama_no
    #
    #             if 'work_location_id' in self.template:
    #                 if self.employee.work_location_id.name:
    #                     work_location_id = self.template.replace('work_location_id', str(self.employee.work_location_id.name))
    #                     self.template = work_location_id
    #                 else:
    #                     work_location_id = self.template.replace('work_location_id', '')
    #                     self.template = work_location_id
    #
    #
    #
    ...
    def on_change_employees(self):
        ...
        if self.old_document_type_id != self.document_type.id:
            self.template = self.document_type.template
            self.old_document_type_id = self.document_type.id
            self.old_employee_id = False
        ...
        if self.template:
            if self.old_employee_id:
                if self.old_employee_id.name in self.template:
                    self.template = self.template.replace(self.old_employee_id.name, self.employee.name or '')
                if str(self.old_employee_id.name_arabic) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.name_arabic),
                                                          str(self.employee.name_arabic or ''))
                if str(self.old_employee_id.passport_id) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.passport_id),
                                                          str(self.employee.passport_id or ''))
                if str(self.old_employee_id.mobile_phone) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.mobile_phone),
                                                          str(self.employee.mobile_phone or ''))
                if str(self.old_employee_id.country_id.name) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.country_id.name),
                                                          str(self.employee.country_id.name or ''))
                if str(self.old_employee_id.eid) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.eid), str(self.employee.eid or ''))
                if str(self.old_employee_id.visa_no) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.visa_no),
                                                          str(self.employee.visa_no or ''))
                if str(self.old_employee_id.visa_expire) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.visa_expire),
                                                          str(self.employee.visa_expire or ''))
                if str(self.old_employee_id.permit_no) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.permit_no),
                                                          str(self.employee.permit_no or ''))
                if str(self.old_employee_id.job_id.name) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.job_id.name),
                                                          str(self.employee.job_id.name or ''))
                if str(self.old_employee_id.company_id.name) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.company_id.name),
                                                          str(self.employee.company_id.name or ''))
                if str(self.old_employee_id.joining_date) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.joining_date),
                                                          str(self.employee.joining_date or ''))
                if self.old_employee_id.work_email in self.template:
                    self.template = self.template.replace(self.old_employee_id.work_email,
                                                          self.employee.work_email or '')
                if str(self.old_employee_id.contract_id.wage) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.contract_id.wage),
                                                          str(self.employee.contract_id.wage or ''))
                if self.old_employee_id.department_id.name in self.template:
                    self.template = self.template.replace(self.old_employee_id.department_id.name,
                ...
                if self.old_employee_id and self.old_employee_id.department_id and self.old_employee_id.department_id.name in self.template:
                    employee_department = self.template.replace(self.old_employee_id.department_id.name,self.employee.department_id.name)
                    self.template = employee_department
                ...
                if str(self.old_employee_id.contract_id.wage) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.contract_id.wage),
                                                          str(self.employee.contract_id.wage or ''))
                if str(self.old_employee_id.employee_no) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.employee_no),
                                                          str(self.employee.employee_no or ''))
                if str(self.date) in self.template:
                    self.template = self.template.replace(str(self.date), str(self.date or ''))
                if str(self.old_employee_id.birthday) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.birthday),
                                                          str(self.employee.birthday or ''))
                if str(self.old_employee_id.passport_expiry_date) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.passport_expiry_date),
                                                          str(self.employee.passport_expiry_date or ''))
                if str(self.old_employee_id.iqama_no) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.iqama_no),
                                                          str(self.employee.iqama_no or ''))
                if str(self.old_employee_id.work_location_id.name) in self.template:
                    self.template = self.template.replace(str(self.old_employee_id.work_location_id.name),
                                                          str(self.employee.work_location_id.name or ''))
                ...
                placeholders = {
                    ...
                    'name_ar': self.employee.name_arabic,
                    'designation_id': str(self.employee.job_id.name),
                    'passport_no': self.employee.passport_id,
                    'employee_mobile_phone': self.employee.mobile_phone,
                    'nationality': str(self.employee.country_id.name),
                    'emp_eid': self.employee.eid,
                    'emp_visa_no': self.employee.visa_no,
                    'emp_visa_expiry': str(self.employee.visa_expire),
                    'emp_permit_no': self.employee.permit_no,
                    'net_salary': str(self.employee.contract_id.wage),
                    'employee_email': self.employee.work_email,
                    'employee_department': self.employee.department_id.name,
                    'company_ids': self.employee.company_id.name,
                    'join_date': str(self.employee.joining_date),
                    'employee_wage': str(self.employee.contract_id.wage),
                    'employee_no': str(self.employee.employee_no),
                    'date': str(self.date),
                    'birthday': str(self.employee.birthday),
                    'passport_expiry_date': str(self.employee.passport_expiry_date),
                    'iqama_no': str(self.employee.iqama_no),
                    'work_location_id': str(self.employee.work_location_id.name),
                }

                for placeholder, value in placeholders.items():
                    if placeholder in self.template:
                        self.template = self.template.replace(placeholder, value if value else '')

        try:
            self.template_value = TAG_RE.sub('', str(self.template))
        except Exception as e:
            raise UserError(f"Error processing template: {str(e)}")
    ...
    def send_email(self):
        self.ensure_one()
        template_id = self._find_mail_template(force_confirmation_template=False)
