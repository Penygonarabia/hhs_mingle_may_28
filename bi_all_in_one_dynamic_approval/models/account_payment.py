...
class AccountPayment(models.Model):
    ...
    def default_get(self, fields):
        ...
        if self.env.user.company_id.dynamic_approval:
            res['suggested_ids'] = False
            model_approval_id = self.env['model.approval'].sudo().search([('model_id', '=', 'account.payment'), ('active', '=', True)])
    ...
    payment_approval_id = fields.Many2one(
    ...
    current_waiting_approval_line_id = fields.Many2one(
        'account.payment.approved', string='Current approval line', copy=False)
    payment_approved_ids = fields.One2many(
        'account.payment.approved', 'order_id', string='Sale Approval Details')
    current_approval_state = fields.Boolean(
        'Current approval state', copy=False)
    approved_user_ids = fields.Many2many(
        'res.users',  'approved_user_account_payment_rel', string='Approved Users', copy=False)
    ...
    def create(self, values):
        ...
        for res in result:
            if res.is_approval and not res.payment_approval_id and res.suggested_ids and res.state == 'draft':
                data = res.suggested_ids.filtered(lambda l: l.minimum_amount <= res.amount)
                if data:
                    new_approval_id = data.sorted(key=lambda l: l.minimum_amount, reverse=True)[0].id
                    res.payment_approval_id = new_approval_id
                else:
                    raise ValidationError('You need to add approvals for create this payment!')
        return result

    def write(self, values):
        approval = False
        for record in self:
            approval = record.payment_approval_id
        result = super(AccountPayment, self).write(values)
        for record in self:
            if not record.payment_approval_id and approval:
                if record.is_approval and not record.payment_approval_id and record.suggested_ids and record.state == 'draft':
                    data = record.suggested_ids.filtered(lambda l: l.minimum_amount <= record.amount)
                    if data:
                        new_approval_id = data.sorted(key=lambda l: l.minimum_amount, reverse=True)[0].id
                        record.payment_approval_id = new_approval_id
                    else:
                        raise ValidationError('You need to add approvals for create this payment!')
        return result
    ...
    def _compute_suggested_ids(self):
        for record in self:
            ...
            if record.is_approval:
                ...
                if record.suggested_ids and record.state == 'draft' and record.company_id.approval_type:
                    data = record.suggested_ids.filtered(lambda l: l.minimum_amount <= record.amount)
                    if data:
                        new_approval_id = data.sorted(key=lambda l: l.minimum_amount, reverse=True)[0].id
                        record.payment_approval_id = new_approval_id
    ...
    @api.depends('payment_approved_ids')
    def _compute_all_level_approved(self):
        for record in self:
            if record.payment_approved_ids and all(record.payment_approved_ids.mapped('state')):
    ...
    def action_reject(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'account.payment.reject',
            'target': 'new',
    ...
    def action_post(self):
        for current in self:
            if current.is_approval and current.payment_approval_id:
                if current.payment_approval_id and not current.payment_approved_ids:
                    if not current.payment_approval_id.approval_line_ids:
                        raise ValidationError(_('No any approval level found!'))
                    approval_lines = []
                    for record in current.payment_approval_id.approval_line_ids:
                        approval_lines.append(
                            (0, 0, current._prepare_approved_line(record)))
                    if approval_lines:
                        current.write({'payment_approved_ids': approval_lines})
                        template = self.env.ref(
                            'bi_all_in_one_dynamic_approval.request_approval_email_template_payment')
                        if template:
                            mail = template.send_mail(int(current.id))
                            if mail:
                                mail_id = self.env['mail.mail'].browse(mail)
                                mail_id[0].sudo().send()
                if current.payment_approved_ids:
                    if not all(current.payment_approved_ids.mapped('state')):
                        approval_level = 0
                        for approval in current.payment_approved_ids.filtered(lambda l: not l.state):
                        ...
                        submit_template = self.env.ref(
                            'bi_all_in_one_dynamic_approval.submit_for_approval_email_template_payment')
                        if submit_template:
                        ...
                        if current.all_level_approved:
                            template = self.env.ref(
                                'bi_all_in_one_dynamic_approval.approved_account_payment_email_template')
                            if template:
                                if current.is_saleperson_in_cc:
    ...
    @api.onchange('payment_approval_id')
    def onchange_payment_approval_id(self):
        if self.payment_approval_id:
            if self.payment_approval_id.minimum_amount > self.amount:
