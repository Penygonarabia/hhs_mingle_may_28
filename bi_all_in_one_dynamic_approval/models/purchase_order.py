...
class PurchaseOrder(models.Model):
    ...
    def default_get(self, fields):
        ...
        if self.env.user.company_id.dynamic_approval:
            model_approval_id = self.env['model.approval'].sudo().search([('model_id', '=', 'purchase.order'), ('active', '=', True)])
    ...
    purchase_approval_id = fields.Many2one(
    ...
    current_waiting_approval_line_id = fields.Many2one(
        'purchase.order.approved', string='Current approval line', copy=False)
    purchase_approved_ids = fields.One2many(
        'purchase.order.approved', 'order_id', string='Purchase Approval Details')
    ...
    approved_user_ids = fields.Many2many(
        'res.users',  'approved_user_purchase_order_rel', string='Approved Users', copy=False)
    ...
    def create(self, values):
        ...
        for res in result:
            if res.is_approval and not res.purchase_approval_id and res.suggested_ids and res.state == 'draft' and res.company_id.approval_type:
                if res.company_id.approval_type == 'before_tax_amount':
                    ...
                    if data:
                        ...
                        res.purchase_approval_id = new_approval_id
                    else:
                        raise ValidationError('You need to add approvals for create this purchase order!')
    ...
    def write(self, values):
        approval = False
        for record in self:
            approval = record.purchase_approval_id
        result = super(PurchaseOrder, self).write(values)
        for record in self:
            if not record.purchase_approval_id and approval:
                if record.is_approval and not record.purchase_approval_id and record.suggested_ids and record.state == 'draft' and record.company_id.approval_type:
                    if record.company_id.approval_type == 'before_tax_amount':
                        ...
                        if data:
                            new_approval_id = data.sorted(key=lambda l: l.minimum_amount, reverse=True)[0].id
                            record.purchase_approval_id = new_approval_id
                        else:
                            raise ValidationError('You need to add approvals for create this purchase order!')
                ...
                if record.order_line and record.suggested_ids and record.state == 'draft' and record.company_id.approval_type:
                    if record.company_id.approval_type == 'before_tax_amount':
                        ...
                        if data:
                            ...
                            record.purchase_approval_id = False
    ...
    @api.depends('purchase_approved_ids')
    def _compute_all_level_approved(self):
        for record in self:
            if record.purchase_approved_ids and all(record.purchase_approved_ids.mapped('state')):
    ...
    def action_reject(self):
        ...
        return {
            'name': _('Purchase Order Reject'),
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'purchase.order.reject',
            'target': 'new',
    ...
    def button_confirm(self):
        ...
        for current in self:
            if current.is_approval and current.purchase_approval_id:
                if current.purchase_approval_id and not current.purchase_approved_ids:
                    if not current.purchase_approval_id.approval_line_ids:
                        raise ValidationError(_('No any approval level found!'))
                    approval_lines = []
                    for record in current.purchase_approval_id.approval_line_ids:
                        approval_lines.append(
                            (0, 0, current._prepare_approved_line(record)))
                    if approval_lines:
                        current.write({'purchase_approved_ids': approval_lines,
                                   'is_saleperson_in_cc': current.purchase_approval_id.is_sale_person})
                        template = self.env.ref(
                            'bi_all_in_one_dynamic_approval.request_approval_email_template_purchase')
                        if template:
                            ...
                            if mail:
                                ...
                                mail_id[0].sudo().send()
                if current.purchase_approved_ids:
                    if not all(current.purchase_approved_ids.mapped('state')):
                        approval_level = 0
                        for approval in current.purchase_approved_ids.filtered(lambda l: not l.state):
                        ...
                        submit_template = self.env.ref(
                            'bi_all_in_one_dynamic_approval.submit_for_approval_email_template_purchase')
                        if submit_template:
                        ...
                        if current.all_level_approved:
                            template = self.env.ref(
                                'bi_all_in_one_dynamic_approval.approved_purchase_order_email_template')
                            if template:
                                if current.is_saleperson_in_cc:
                                    template.email_cc = current.user_id.email if current.user_id else False
                                mail = template.send_mail(int(current.id))
                                if mail:
                                    mail_id = self.env['mail.mail'].browse(mail)
                                    mail_id[0].sudo().send()
                        current.button_approve()
                        return res
            else:
                current.button_approve()
                return res

    @api.onchange('purchase_approval_id')
    def onchange_sale_approval_id(self):
        if self.purchase_approval_id:
            if self.company_id.approval_type and self.company_id.approval_type == 'total' and self.purchase_approval_id.minimum_amount > self.amount_total:
                raise ValidationError(
                    _('Selected Approval is not proper as per your order!'))
            if self.company_id.approval_type and self.company_id.approval_type == 'before_tax_amount' and self.purchase_approval_id.minimum_amount > self.amount_untaxed:
                raise ValidationError(
                    _('Selected Approval is not proper as per your order!'))
