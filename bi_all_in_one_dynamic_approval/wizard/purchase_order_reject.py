...
class PurchaseOrderReject(models.TransientModel):
    ...
    def action_reject_purchase_order(self):
        ...
        if self.order_id and self.order_id.current_waiting_approval_line_id and not self.order_id.current_waiting_approval_line_id.state and not self.order_id.current_approval_state:
            ...
            template = self.env.ref('bi_all_in_one_dynamic_approval.reject_approval_email_template_purchase')
            if template:
