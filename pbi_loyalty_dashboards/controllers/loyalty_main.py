...
# Region/city for the Loyalty Analysis charts come from the legacy ERP
# customer master, NOT from res_partner.work_center_group_id/work_center_id
# (the loyalty_dashboard module's res.partner.customer_city_id -> res.city ->
# def_work_center_id -> work.center.location chain — an admin-set mapping
# that drifts from the real data, e.g. customer VR005 has cst_subregion
# 'RYD' (Riyadh/Central) but was showing as Western/Jeddah).
#
# Mapping plan: transaction_header.trnh_cstno -> res_partner.ref ->
# customer.cst_no -> customer.cst_subregion -> t_subregions.sr_code, then
# t_subregions.sr_region -> t_regionsdesc.r_code (r_lang=1) = REGION, and
# t_subregionsdesc.sr_code (sr_lang=1) = CITY. cst_subregion is
# inconsistently cased in the source data ('RYD'/'ryd'/'Khm'), hence the
# UPPER(TRIM(...)) on the join.
REGION_CITY_JOIN = """
    LEFT JOIN customer c ON c.cst_no = p.ref
...
"""
REGION_DIM = ("INITCAP(rd.r_desc)", "sub.sr_region")
CITY_DIM = ("INITCAP(srd.sr_desc)", "sub.sr_code")
...
class PbiLoyaltyDashboardController(http.Controller):
    ...
    def _has_access(self):
        return menu_allowed("pbi_dashboards.menu_pbi_loyalty_analysis")
    ...
    def _tier_rows(self, level, date_from, date_to, drill_region_id=None, region_name=None):
        dim_col, dim_id = {"region": REGION_DIM, "city": CITY_DIM}[level]
        clauses = [
        ...
        if level == "city" and drill_region_id:
            clauses.append("sub.sr_region = %s")
        ...
        for r in rows:
            ...
            if key not in buckets:
                buckets[key] = {"label": r["label"], "id": r["id"], "ids": set()}
                order.append(key)
            buckets[key][r["tier"]] = buckets[key].get(r["tier"], 0) + int(r["cnt"])
            buckets[key]["ids"].update(r["ids"] or [])
        result = []
        for key in order:
            row = buckets[key]
            row["ids"] = sorted(row["ids"])
            for t in tiers:
                row.setdefault(t, 0)
            result.append(row)
    ...
    def _newusers_rows(self, level, date_from, date_to, drill_region_id=None, region_name=None):
        ...
        where = " AND ".join(clauses)
        rows = self._query(f"""
            SELECT {dim_col} AS label, {dim_id} AS id, count(*) AS value, array_agg(p.id) AS ids
            FROM res_partner p
            {REGION_CITY_JOIN}
            WHERE {where}
            GROUP BY 1, 2
            ORDER BY 3 DESC
        """, params)
        return [{"label": r["label"], "id": r["id"], "value": int(r["value"] or 0), "ids": r["ids"] or []}
                for r in rows]
