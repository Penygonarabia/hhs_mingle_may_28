...
function groupedBarChart(el, data, seriesKeys, seriesColors, seriesLabels, opts = {}) {
  ...
  // What the VALUE LABELS need. They are centred on the bar and as wide as
  // their text, so a group has to hold nSeries of them at that pitch plus a
  // gap's worth of slack for the outermost one to overhang its bar. Without
  // this the chart simply took its container's width and crushed however many
  // categories the drill returned into it.
  const labelPitch = widestValueLabel(el, data, seriesKeys, fmtCompact) + VALUE_LABEL_GAP;
  const naturalW = opts.width || el.clientWidth || 520;
  const W = Math.max(naturalW, data.length * Math.ceil(nSeries * labelPitch + gap) + marginL + marginR);
  // Still width="100%" while the container is big enough — that is this
  // board's normal state and stays pixel-identical. Only a chart the labels
  // have actually pushed wider is pinned and scrolled (.chart-scroll).
  ...
  // House chart design, matching pbi_chart_lib's groupedBarChart: the bars
  // inside a set are held apart, and they take a share of the slot so what is
  // left over reads as the gap to the next set. 0.73 is the preferred fill
  // rather than a cap — spare room goes into the bar before the chart has to
  // get wider — and barPitch stays separate from barW so the 120px cap cannot
  // pull two labels back into each other.
  ...
  data.forEach((d, gi) => {
    ...
    seriesKeys.forEach((k, si) => {
      ...
      svg += `<rect data-series-idx="${si}" data-tip="${d.label}||${seriesLabels[si]}||${val}" rx="3" ry="3" x="${x}" y="${y}" width="${barW}" height="${Math.max(barH, 1)}" fill="${seriesColors[si % seriesColors.length]}" style="cursor:pointer"/>`;
      svg += `<text class="bar-value" data-series-idx="${si}" x="${x + barW / 2}" y="${y - 3}" text-anchor="middle">${fmtCompact(val)}</text>`;
    });
...
export class PbiLoyaltyDashboard extends Component {
  ...
  redirect(key, row) {
    ...
    if (leaf === 'city') {
      ...
      // the customer list for the selected city. `row.ids` is the exact set
      // of res.partner ids the chart bucket was computed from (region/city
      // come from the ERP customer/subregion mapping, not a res.partner
      // field), so filter by id rather than any region/city column.
      const domain = [['id', 'in', row.ids || []]];
  ...
  renderSection(key) {
    ...
    if (cfg.multiSeries) {
      ...
      // Over the card, not the chart: this template keeps the legend in its
      // own node beside the chart rather than inside it.
      attachLegendFilter(r.legend.el.closest('.pbi-card'), 'bar');
