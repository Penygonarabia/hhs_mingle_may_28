...
class DbModelTaskMessageLogAnalysis(models.Model):
    _name = "dbmodel.task.message.log.analysis.ct"
    _description = "DBM Task Message Log Analysis (CT)"
    _table = "dbmodel_task_message_log_analysis_ct"
    _auto = False
    ...
    def refresh_materialized(self):
        ...
        self.env.cr.execute(
            "REFRESH MATERIALIZED VIEW CONCURRENTLY dbmodel_task_message_log_analysis_ct"
        )
    ...
    def init(self):
        ...
        cr.execute("""
            ...
            DECLARE r record;
            BEGIN
                SELECT relkind INTO r FROM pg_class WHERE relname = 'dbmodel_task_message_log_analysis_ct';
                IF FOUND THEN
                    IF r.relkind = 'm' THEN
                        EXECUTE 'DROP MATERIALIZED VIEW dbmodel_task_message_log_analysis_ct CASCADE';
                    ELSE
                        EXECUTE 'DROP VIEW dbmodel_task_message_log_analysis_ct CASCADE';
                    END IF;
                END IF;
            END $$;
        """)
        ...
        cr.execute("""
            CREATE MATERIALIZED VIEW dbmodel_task_message_log_analysis_ct AS
            WITH src AS (
        ...
        cr.execute("CREATE UNIQUE INDEX IF NOT EXISTS dbmodel_tml_analysis_ct_id_idx ON dbmodel_task_message_log_analysis_ct (id)")
        cr.execute("CREATE INDEX IF NOT EXISTS dbmodel_tml_analysis_ct_user_id_idx ON dbmodel_task_message_log_analysis_ct (user_id)")
        cr.execute("CREATE INDEX IF NOT EXISTS dbmodel_tml_analysis_ct_status_idx ON dbmodel_task_message_log_analysis_ct (ptml_final_taskstatus)")
        cr.execute("CREATE INDEX IF NOT EXISTS dbmodel_tml_analysis_ct_user_status_idx ON dbmodel_task_message_log_analysis_ct (user_id, ptml_final_taskstatus)")
        cr.execute("CREATE INDEX IF NOT EXISTS dbmodel_tml_analysis_ct_task_id_idx ON dbmodel_task_message_log_analysis_ct (task_id)")
        cr.execute("CREATE INDEX IF NOT EXISTS dbmodel_tml_analysis_ct_task_date_idx ON dbmodel_task_message_log_analysis_ct (task_date)")
        cr.execute("ANALYZE dbmodel_task_message_log_analysis_ct")
