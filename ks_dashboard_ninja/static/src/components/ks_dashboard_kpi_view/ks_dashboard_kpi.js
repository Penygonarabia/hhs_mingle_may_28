...
export class Ksdashboardkpiview extends Component {
    ...
    prepareKpiData() {
        ...
        if (field.name && (field.name === "KPI AVG RTAT" || field.name === "AVG RTAT")) {
            const formatDecimalHours = (val) => {
                if (val === undefined || val === null || val === false) return "";
                if (typeof val === "number" || !isNaN(parseFloat(val))) {
                    let numericVal = parseFloat(val);
                    let sign = numericVal < 0 ? "-" : "";
                    let absVal = Math.abs(numericVal);
                    let hrs = Math.floor(absVal);
                    let mins = Math.round((absVal - hrs) * 60);
                    if (mins === 60) {
                        hrs += 1;
                        mins = 0;
                    }
            ...
            item_info['count_1'] = formatDecimalHours(kpi_data[0]['record_data']);
            item_info['count_1_tooltip'] = formatDecimalHours(kpi_data[0]['record_data']);
            if (kpi_data[0].target) {
                item_info['target'] = formatDecimalHours(kpi_data[0].target);
            }
            if (kpi_data[1]) {
                item_info['count_2'] = formatDecimalHours(kpi_data[1]['record_data']);
                if (this.field.ks_data_comparison === 'None') {
                    item_info['count'] = formatDecimalHours(count_1) + "/" + formatDecimalHours(count_2);
                    item_info['count_tooltip'] = item_info['count'];
                }
            }
            this.state.item_info_kpi1 = this.state.item_info_kpi3 = item_info;
            if (kpi_data[1]) {
