...
class HrCustody(models.Model):
    ...
    def renew_approve(self):
        ...
        for custody in self.env['hr.custody'].search([('custody_name', '=', self.custody_name.id),('state','=','approved')]):
            if custody.employee.id == self.employee.id and custody.custody_name.display_name == self.custody_name.display_name:
    ...
    def approve(self):
        for custody in self.env['hr.custody'].search([('custody_name', '=', self.custody_name.id),('state','=','approved')]):
            if custody.employee.id == self.employee.id and custody.custody_name.display_name == self.custody_name.display_name:
                raise UserError(_("Custody Items Already Exist"))
        self.state = 'approved'
    ...
    # @api.onchange('asset_types')
    # def _asset_name (self):
    #     for rec in self:
    #         res = {}
    #         vals = []
    #         rec.custody_name = ""
    #         asset_search = self.env['account.asset.asset'].search([('category_id', '=', rec.asset_types.id)])
    #         for asset in asset_search:
    #             print("asset",asset.name)
    #             vals.append(asset.id)
    #             res['domain'] = {'custody_name': [('id', 'in', vals)]}
    #         return res
    ...
    def _asset_name(self):
        for rec in self:
            ...
            for asset in asset_search:
                vals.append(asset.id)
            return {'domain': {'custody_name': [('id', 'in', vals)]}}
    ...
    employee = fields.Many2one('hr.employee', string='Employee', required=True, help="Employee",
                               ...
                               domain="[('exit_date', '=', False)]")
    purpose = fields.Char(string='Reason', tracking=True, required=True, help="Reason")
    # custody_name = fields.Many2one('custody.property', string='Property', required=True, readonly=True,
    #                                help="Property name",
    #                                states={'draft': [('readonly', False)]}
    #                                )
    asset_types = fields.Many2one('account.asset.category', string='Asset Type', required=True,
                                   help="Asset Type name")

    custody_name = fields.Many2one('account.asset.asset', string='Property', required=True,
                                   help="Property name")
    ...
    renew_date = fields.Date(string='Renewal Return Date', tracking=True,
                             help="Return date for the renewal", copy=False)
    ...
    def _onchange_employee(self):
        for rec in self:
            ...
            if rec.employee:
                rec.employee_number = rec.employee.employee_no
...
class HrPropertyName(models.Model):
    ...
    # product_id = fields.Many2one('account.asset.category', string='Product', help="Product")
    product_id = fields.Many2one('account.asset.asset', string='Product', help="Product")
