...
class HrCustody(models.Model):
    ...
    #         equipment_ids = []
    #         print("equipment_ids",equipment_ids)
    #         for each1 in equipment_obj:
    #             print("each1",each1.custody_name.id)
    ...
    #Newly Added in 22-Apr-2024
    def _equipment_count(self):
        for record in self:
            equipment_obj = self.env['hr.custody'].search([('employee', '=', record.id), ('state', '=', 'approved')])
            if equipment_obj:
                equipment_ids = equipment_obj.mapped('custody_name.id')
                record.equipment_count = len(equipment_ids)
            else:
