...
class dbm_spareparts_warranty(models.Model):
    _name = 'dbm.spareparts.warranty'
    _description = 'dbm Spare Parts Warranty'
    _auto = False
        ...
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW %s AS (
                ...
                FROM
                    ...
                    LEFT JOIN service_warranty sw 
                        ON sw.id = pt.service_warranty_id
                    LEFT JOIN work_center_location wcl 
                        ON wcl.id = pt.work_center_id
                    LEFT JOIN work_center_group wcg 
                        ON wcg.id = pt.work_center_group_id
                    LEFT JOIN aggregated_properties ap 
                        ON ap.project_task_id = pt.id
                WHERE 
                    pt.active = true
