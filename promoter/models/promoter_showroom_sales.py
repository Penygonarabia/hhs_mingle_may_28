...
class PromoterShowroomSales(models.Model):
    _name = 'promoter.showroom.sales'
    ...
    # @api.depends('promoter_mobile_user_bool')
    ...
    #             )
    #             rec.product_category_id = promoter_assignment.category_id.id if promoter_assignment else False
    #         else:
    ...
    #     for rec in self:
    #         if rec.promoter_mobile_user_bool:
    #             # Mobile user → auto-fill from promoter assignment
    #             promoter_assignment = self.env['promoter.assignment'].search(
    #                 [('promoter_id', '=', self.env.user.id)], limit=1
    #             )
    #             rec.product_category_id = promoter_assignment.category_id.id if promoter_assignment else False
    ...
    # def _inverse_product_category_id(self): allowed_group_is_promoter
    ...
    attachment_ids = fields.Many2many(
        'ir.attachment',
        'promoter_shop_sales_attachment_rel',
        'sales_id',
        'attachment_id',
        string='Attachments'
    )
    ...
    promoter_access_bool = fields.Boolean(string ="promoter_access_bool",compute="_compute_promoter_access_bool",default=False)
    ...
    #     for rec in self:
    #         rec.product_category_readonly = rec.promoter_mobile_user_bool

    @api.depends('promoter_mobile_user_bool')
    def _compute_promoter_access_bool(self):
        for rec in self:
            rec.promoter_access_bool = bool(rec.promoter_mobile_user_bool)
        ...
        for rec in self:
            if rec.promoter_mobile_user_bool:
              ...
              [('promoter_id', '=', self.env.user.id)], limit=1 ) 
                ...
                rec.showroom = promoter_assignment.showroom_id.id                
                rec.assignment_id =promoter_assignment.id
                rec.product_category_id=promoter_assignment.category_id.id    
                # Make field readonly for mobile users
                # rec._fields['product_category_id'].readonly = True
                # print("...........promoter_assignment........",promoter_assignment.promoter_id.name ,rec._fields['product_category_id'].readonly)
    ...
    @api.constrains("current_latitude", "current_longitude", "showroom", "city_id", "district_id")
    def _check_geo_location(self):
        """Validate promoter's location against showroom coordinates."""
        ...
        for rec in self:
            # Get promoter coordinates (from record or user)
            ...
            if not rec.showroom or not rec.showroom.latitude or not rec.showroom.longitude:
            ...
            distance = haversine(user_lat, user_lon, rec.showroom.latitude, rec.showroom.longitude)

            wfo_radius = int(
            self.env['ir.config_parameter'].sudo().get_param('promoter.promoter_wfo_radius', default=0)
            )
    ...
    @api.depends('showroom')
    def _compute_location_fields(self):
        for rec in self:
            if rec.showroom:
                rec.city_id = rec.showroom.city.id if rec.showroom.city else False
                rec.district_id = rec.showroom.district.id if rec.showroom.district else False
                rec.region_id = rec.showroom.region_id.id if rec.showroom.region_id else False
            else:
                rec.city_id = False
    ...
    def _onchange_available_user(self):
        for rec in self:
            if rec.assignment_id:
                print("rec.promoter_id.promoter_id.user_id.id", rec.assignment_id.promoter_id.id)
                rec.user_id = rec.assignment_id.promoter_id.id    
    ...
    def search_fetch(self, domain, field_names, offset=0, limit=None, order=None):
        ...
        if (user.has_group('promoter.group_promoter_user')):
            domain += [
                 ('dealer_id', '=', user.dealer_id.id),('showroom','=',user.showroom_id.id)
            ]
            return super(PromoterShowroomSales, self).search_fetch(domain, field_names, offset, limit, order)        
    ...
    def create(self, vals):
        ...
        # If we have assignment_id, check the promoter's attendance for today
        if vals.get("assignment_id"):
            assignment = self.env["promoter.assignment"].browse(vals["assignment_id"])
            promoter_user = assignment.promoter_id
            if promoter_user:
                # Get linked HR Employee from res.users
                employee = promoter_user.employee_id
                if not employee:
                    raise ValidationError(
                        _("No employee record linked to promoter user %s.") % promoter_user.name
                    )
                ...
                if attendance:
                    _logger.info("🧾 Found attendance for %s (Employee %s): check_in=%s check_out=%s",
                                promoter_user.name, employee.name, attendance.check_in, attendance.check_out)
                else:
                    _logger.info("🧾 No attendance found for promoter %s (Employee %s) today",
                                promoter_user.name, employee.name)
                ...
                raise ValidationError(_("Invalid assignment / promoter for this sales entry."))

        # proceed with normal creation and target update
        record = super(PromoterShowroomSales, self).create(vals)
    ...
    def write(self, vals):
        ...
        # Restrict promoter users to modify only today's records (server UTC only)
        if self.env.user.has_group("promoter.group_promoter_user"):
            ...
            for record in self:
                ...
                if record.assignment_id and record.assignment_id.promoter_id:
                    attendance = self.env['hr.attendance'].search([
                        ('employee_id', '=', record.assignment_id.promoter_id.id),
        ...
        target_obj = self.env['sales.target']
        old_data = []
    ...
    def unlink(self):
        """Allow promoter (mobile) users to delete only today's records.
    ...
    def _get_target_key(self, record):
        ...
        return [
            ...
            ('showroom_id', '=', safe(record.showroom)),
            ('promoter_id', '=', record.assignment_id.promoter_id.id if record.assignment_id and record.assignment_id.promoter_id else False),
    ...
    def _create_target(self, record):
        ...
        promoter = record.assignment_id.promoter_id if record.assignment_id else None
        user = promoter.user_id if promoter else None
        promoter_id = safe_id(promoter)
        promoter_code = f"pr_{getattr(user, 'user_code', '')}" if user else ""
        promoter_name = promoter.name if promoter else ""
        ...
        showroom_id = safe_id(record.showroom)
        ...
        return self.env['sales.target'].create(vals)
    ...
    def default_get(self, fields_list):
        ...
        if not user.has_group("promoter.group_promoter_user"):
            return res
        ...
        if not user_lat or not user_lon:
            raise ValidationError(
                _("Cannot validate location for promoter '%s': Coordinates missing.") % user_name
            )

        # Get the assigned showroom for this promoter
        promoter_assignment = self.env['promoter.assignment'].search(
            [('promoter_id', '=', user.id),('active', '=', True)], limit=1
        )
        if not promoter_assignment or not promoter_assignment.showroom_id:
            raise ValidationError(
                _("Cannot validate location for promoter '%s': No showroom assigned.") % user_name
            )
        ...
        if not showroom.latitude or not showroom.longitude:
            raise ValidationError(
                _("Cannot validate location for promoter '%s': Showroom coordinates missing.") % user_name
            )
        if not showroom.city:
            raise ValidationError(
                _("Geo-location validation failed for promoter '%s': Missing city for showroom.") % user_name
            )

        # --- SAFE conversion of stored config parameter (handles '7400.86', '7400', None, etc.)
        radius_param = self.env['ir.config_parameter'].sudo().get_param('promoter.promoter_wfo_radius', '0')
        try:
