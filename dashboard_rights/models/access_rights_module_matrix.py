...
class AccessRightsModuleMatrix(models.TransientModel):
    ...
    def action_view_rights_summary(self):
        ...
        return {
            ...
            "view_mode": "tree,form",
            "views": [
                [self.env.ref("dashboard_rights.view_dashboard_rights_menu_list").id, "tree"],
                [self.env.ref("dashboard_rights.view_dashboard_rights_menu_form").id, "form"],
            ],
            "domain": [("user_id", "=", self.user_id.id), ("menu_id", "in", menu_ids)],
            "search_view_id": [self.env.ref("dashboard_rights.view_dashboard_rights_menu_search").id, "search"],
    ...
    def _reload_self(self):
        view_id = self.env.ref(
            "dashboard_rights.view_access_rights_module_matrix_form"
        ).id
        return {
            "type": "ir.actions.act_window",
