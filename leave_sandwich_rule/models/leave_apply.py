...
class LeaveApply(models.Model):
    ...
    request_date_from = fields.Date(default=datetime.today())
    request_date_to = fields.Date(default=datetime.today())

    @api.constrains('request_date_from','request_date_to')
    def _validate_check_date_from(self):
        for rec in self:
            if rec.request_date_from and rec.request_date_to:
                if rec.request_date_from >= rec.request_date_to:
                    raise ValidationError("Expected Return Date is always greater than Leave Date from")
    ...
    def _compute_number_of_days_display(self):
        ...
        return super(LeaveApply, self)._compute_number_of_days_display()
