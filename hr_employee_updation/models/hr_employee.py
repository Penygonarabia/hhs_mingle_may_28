...
class HrEmployee(models.Model):
    ...
    @api.constrains('joining_date','hiring_date')
    def _check_joing_date_and_hiring_date(self):
        for rec in self:
            if rec.joining_date and rec.hiring_date:
                if rec.joining_date < rec.hiring_date:
                    raise ValidationError("Joining Date is always greater than Hiring Date")
