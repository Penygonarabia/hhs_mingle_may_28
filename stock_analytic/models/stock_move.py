...
class StockMove(models.Model):
    ...
    on_hand_quantity = fields.Float(string="O/H Qty")
    
    @api.onchange('product_id')
    def _onchange_product(self):
        if self.product_id:
            if self.picking_id.picking_type_id.warehouse_id.analytic_id:
                self.analytic_distribution = {str(self.picking_type_id.warehouse_id.analytic_id.id):100}
            else:
...
class StockMoveLine(models.Model):
    ...
    def _prepare_stock_move_vals(self):
        ...
        """
        res = super(StockMoveLine,self)._prepare_stock_move_vals()
        if self.analytic_distribution:
            res.update({"analytic_distribution": self.analytic_distribution})
        return res
