...
class HrAttendance(models.Model):
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            if vals.get('check_out') and rec.is_promoter:
                ...
                # Skip geo validation for automatic/cron updates
                if self.env.context.get('skip_geo_validation'):
                    print(f"[CHECKOUT VALIDATION] Skipped geo validation for {rec.employee_id.name} (cron or system action)")
                    continue
                ...
                if isinstance(check_out, datetime) and check_out.year != 1900:
                    rec._validate_geo()
    ...
    # -----------------------------
    # CRON: Auto Checkout for Missed Check-outs
    # -----------------------------
    # @api.model
    # def _cron_daily_auto_checkout(self):
    #     """Automatically check out promoters who forgot to check out yesterday."""
    #     from datetime import timedelta
    #     import logging
    ...
    #
    #     # Find all attendances with check_in but no check_out
    #     attendances = self.env["hr.attendance"].search([
    #         ("check_in", "!=", False),
    #         ("check_out", "=", False),
    #     ])
    #
    #     # Filter those whose check_in was yesterday
    #     yesterday_attendances = attendances.filtered(
    #         lambda a: a.check_in.date() == yesterday_date
    #     )
    #
    #     for attendance in yesterday_attendances:
    #         try:
    #             # ✅ Skip geo validation for cron auto-checkout
    #             attendance.with_context(skip_geo_validation=True).check_out = current_time
    #             _logger.info(f"[AUTO CHECKOUT] Successfully checked out {attendance.employee_id.name} at {current_time}")
    #         except Exception as e:
    #             _logger.error(
    #                 f"[AUTO CHECKOUT ERROR] Failed for {attendance.employee_id.name if attendance.employee_id else 'Unknown'}: {str(e)}"
    #             )
    #
