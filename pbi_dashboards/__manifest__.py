...
Access is gated the same way as every other menu under My Dashboard. When
the optional user_menu_rights overlay is installed the menu is hidden until
an admin grants it there; otherwise ordinary Odoo groups decide. Either way
it is enforced both on the menu and on the JSON data route itself. A
post_init_hook seeds an access grant for the bootstrap admin user (only if
that overlay is present) so the feature isn't invisible immediately after
install.
""",
    'author': 'Cielo Digital',
    'depends': [
        # NB: user_menu_rights is deliberately NOT listed. The two modules
        # install and uninstall independently — listing it would drag it in
        # on every pbi_dashboards install and take pbi_dashboards down with
        # it on uninstall. The per-user menu overlay is looked up at runtime
        # instead (controllers/access.py, hooks.py); without it installed
        # these dashboards fall back to normal group-based menu visibility.
        'ks_dashboard_ninja', 'loyalty_dashboard', 'project',
    ...
    'assets': {
        'web.assets_backend': [
            ...
            'pbi_dashboards/static/src/js/sales_kpi_dashboard_q.js',
            'pbi_dashboards/static/src/xml/sales_kpi_dashboard_q.xml',
            'pbi_dashboards/static/src/css/sales_kpi_dashboard_q.css',
            ...
            'pbi_dashboards/static/src/js/sales_analysis_q.js',
            'pbi_dashboards/static/src/xml/sales_analysis_q.xml',
            'pbi_dashboards/static/src/css/sales_analysis_q.css',
