...
class PbiSalesKpiController(http.Controller):
    ...
    def _drill_clauses(self, drill_path):
        ...
        for entry in (drill_path or []):
            ...
            if level not in self.LEVEL_COLUMNS or code is None:
                continue
            code_col, _ = self.LEVEL_COLUMNS[level]
    ...
    def _natural_level(self, drill_path):
        ...
        drilled_idxs = [self.LEVELS.index(e["level"]) for e in (drill_path or [])
                         if isinstance(e, dict) and e.get("level") in self.LEVELS]
        depth = min((max(drilled_idxs) + 1) if drilled_idxs else 0, len(self.LEVELS) - 1)
    ...
    def _fetch_bundle(self, year, month, franchise, customer_group, sales_type_group, drill_path, level_filter_code=None, view_level=None):
        ...
        level = view_level if view_level in self.LEVELS else self._natural_level(drill_path)
        can_drill_further = self.LEVELS.index(level) < len(self.LEVELS) - 1
