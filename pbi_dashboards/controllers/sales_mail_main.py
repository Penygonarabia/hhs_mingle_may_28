...
# Base scope for every Amount/Qty aggregate in this dashboard — which
# franchise-code / product-group combinations are in scope at all,
# independent of whichever one (if any) the Franchise selector narrows to.
# Originally hardcoded to Midea only (MDA), which meant selecting BEKO or
# CANDY in the Franchise filter returned empty data even though real BEKO/
# CANDY sales exist. Now mirrors sales_kpi_main.py's _FRANCHISE_SCOPE so
# the two "Sales" dashboards agree: the MDA branch keeps the exact same
# AC product-family whitelist (PRODUCT_SCOPE_CODES) — so Midea figures are
# unchanged — and BKO/CDY get their own product-group whitelists (identical
# to the sibling Sales Dashboard's). The bi_cstno NOT LIKE 'V%%' dealer-
# account exclusion still applies to all three. With franchise = 'all' the
# dashboard now sums all three franchises instead of Midea only.
...
BASE_SCOPE_SQL = """bi_cstno NOT LIKE 'V%%'
