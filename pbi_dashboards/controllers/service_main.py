...
from .service_config import BOARDS
...
class PbiDashboardBoardEngineMixin:
    ...
    def _has_access(self, board_key):
        ...
        menu = request.env.ref(f"pbi_dashboards.menu_pbi_{board_key}", raise_if_not_found=False)
        if not menu:
            return False
    ...
    def _value_format(self, item_cfg):
        ...
        instead of "393,758", both on tiles and axis ticks."""
    ...
    def _item_payload(self, board_cfg, item_cfg, date_from, date_to, uid):
        value_format = self._value_format(item_cfg)
        if item_cfg.type in ("kpi_single", "kpi_dual"):
            data = service_sql.run_kpi(request.env, uid, board_cfg, item_cfg, date_from, date_to)
            return {"key": item_cfg.key, "name": item_cfg.name, "type": item_cfg.type,
                    "valueFormat": value_format, **data}
        if item_cfg.type in ("bar", "pie"):
            ...
            payload = {"key": item_cfg.key, "name": item_cfg.name, "type": item_cfg.type,
                       "breakdown": breakdown or [], "level": 0, "valueFormat": value_format}
        ...
        if item_cfg.type == "table":
            ...
            return {"key": item_cfg.key, "name": item_cfg.name, "type": "table",
                    "recordModel": item_cfg.record_model,
