...
// Static UI-chrome strings for this dashboard only (page titles, card
// headings, legend/axis words, filter & export labels, toggle-scroll/notes
// scaffolding). Never business-domain vocabulary: franchise brand names
// (Midea/BEKO/CANDY), channel-toggle labels (Dealers/Modern Trade/Whole
// Sale/Projects) and region-toggle labels (Riyadh/Qassim/Western/Eastern)
// mirror real per-code source-system labels, so they're left untranslated,
// same as the "LCAC"/"Split" main-group codes embedded in a couple of
// section headings below. Narrative/notes body text (pbi.dashboard.note,
// keys prefixed sales_mail_) is DB-fetched and never touched here.
addLabels({
  // header/footer
  "Sales Mail Content": "محتوى نشرة المبيعات",
  ...
  // page-nav titles (also reused as page-picker + <h2> section headings)
...
// — see plan revision 2. Drives the page-nav picker/Prev/Next and (for
// toggle pages) the toggle-button row.
const PAGE_DEFS = [
...
export class PbiSalesMailDashboard extends Component {
  static template = "pbi_dashboards.sales_mail_dashboard";
  static props = ["*"];
  ...
  // -- page nav (client-only — no RPC, see renderAll's render-once design) --
  goToPage(idx) { this.state.activePageIndex = Math.max(0, Math.min(this.pages.length - 1, idx)); }
  nextPage() { this.goToPage(this.state.activePageIndex + 1); }
  prevPage() { this.goToPage(this.state.activePageIndex - 1); }
  ...
  exportPptx() {
    ...
    window.open(`/pbi_dashboards/sales_mail/export.pptx?${params}`, '_blank');
  }
  ...
  async importNotes(ev) {
    ...
    try {
      ...
      const response = await fetch('/pbi_dashboards/sales_mail/import_notes', { method: 'POST', body: fd });
  ...
  async _loadImpl(seq) {
    try {
      ...
      const res = await this.rpc('/pbi_dashboards/sales_mail/data', { period, franchise: this.state.franchise });
...
registry.category("actions").add("pbi_dashboards.sales_mail_dashboard", PbiSalesMailDashboard);
