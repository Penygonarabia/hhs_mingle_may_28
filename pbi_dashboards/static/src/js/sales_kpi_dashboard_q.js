...
// "Sales Dashboard (Q)" — sibling of sales_kpi_dashboard_new.js, adapted for transaction tables
// and containing the Salesman level before Customer, and Sales Type Group / Sub-group.
addLabels({
  "Sales Dashboard (Q)": "لوحة مبيعات المبيعات (Q)",
  "Amount": "القيمة",
  "Quantity": "الكمية",
  "Sales Type Group": "مجموعة نوع المبيعات",
  "Sales Type Sub-Group": "المجموعة الفرعية لنوع المبيعات",
  ...
  "Sales Type Sub-Groups": "المجموعات الفرعية لنوع المبيعات",
  "Regions": "المناطق",
  "Salesmen": "مناديب المبيعات",
  "Customers": "العملاء",
  "Product Groups": "مجموعات المنتجات",
  "Product Sub-Groups": "المجموعات الفرعية للمنتجات",
  "All Sales Type Groups": "كل مجموعات نوع المبيعات",
  ...
  "YTD - TY Qty vs LY Qty %": "كمية هذا العام مقابل العام الماضي % - حتى تاريخه (الشهر)",
...
// Updated sales type group options to Retail, Wholesale, Corporate
const SALES_TYPE_GROUP_OPTIONS = [
  { v: 'all', l: 'All' }, { v: 'RT', l: 'Retail' }, { v: 'WS', l: 'Wholesale' }, { v: 'CP', l: 'Corporate' },
];
const SALES_TYPE_GROUP_COLORS = ['--series-1', '--series-2', '--series-3'];

const LEVEL_FILTER_LABELS = {
  salesTypeGroup: 'Sales Type Group', salesTypeSubGroup: 'Sales Type Sub-Group', region: 'Region',
  salesman: 'Salesman', customer: 'Customer', productGroup: 'Product Group', productSubGroup: 'Product Sub-Group',
};
const LEVEL_NAV_LABELS = {
  salesTypeGroup: 'Sales Type Groups', salesTypeSubGroup: 'Sales Type Sub-Groups', region: 'Regions',
  salesman: 'Salesmen', customer: 'Customers', productGroup: 'Product Groups', productSubGroup: 'Product Sub-Groups',
};

const LEVEL_ORDER = ['salesTypeGroup', 'salesTypeSubGroup', 'region', 'salesman', 'customer', 'productGroup', 'productSubGroup'];
...
export class PbiSalesKpiDashboardQ extends Component {
  static template = "pbi_dashboards.sales_kpi_dashboard_q";
  static props = ["*"];
  ...
  setup() {
    ...
    this.state = useState({
      ...
      measure: 'amount',
      drillPath: [], level: 'salesTypeGroup', levelLabel: t('Sales Type Group'), canDrillFurther: true,
  ...
  renderBreadcrumb() {
    ...
    const parts = [{ label: t('All Sales Type Groups'), idx: -1 }, ...this.state.drillPath.map((p, i) => ({ label: p.label, idx: i }))];
  ...
  seriesColors() {
    ...
    return SALES_TYPE_GROUP_COLORS.map(v => style.getPropertyValue(v).trim() || '#2a78d6');
  }
  ...
  async load() {
    ...
    try {
      ...
      const res = await this.rpc('/pbi_dashboards/sales_kpi_q/data', {
        period,
  ...
  renderCharts() {
    ...
    const barColors = [seriesColors[0], seriesColors[1] || '#1baf7a', seriesColors[2] || '#eda100']; // TY / Target / LY
    const isAmount = this.state.measure === 'amount';
...
registry.category("actions").add("pbi_dashboards.sales_kpi_dashboard_q", PbiSalesKpiDashboardQ);
