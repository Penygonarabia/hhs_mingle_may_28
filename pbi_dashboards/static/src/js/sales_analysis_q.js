...
addLabels({
  // header/footer
  "Sales Analysis (Q)": "تحليل المبيعات (ربع سنوي)",
...
export class PbiSalesAnalysisQ extends Component {
  static template = "pbi_dashboards.sales_analysis_q";
  static props = ["*"];
  ...
  exportPptx() {
    ...
    window.open(`/pbi_dashboards/sales_analysis_q/export.pptx?${params}`, '_blank');
  }
  ...
  async importNotes(ev) {
    ...
    try {
      ...
      const response = await fetch('/pbi_dashboards/sales_analysis_q/import_notes', { method: 'POST', body: fd });
  ...
  async _loadImpl(seq) {
    try {
      ...
      const res = await this.rpc('/pbi_dashboards/sales_analysis_q/data', { period, franchise: this.state.franchise });
...
registry.category("actions").add("pbi_dashboards.sales_analysis_q", PbiSalesAnalysisQ);
