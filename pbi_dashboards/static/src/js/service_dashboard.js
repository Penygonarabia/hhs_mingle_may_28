...
addLabels({
  ...
  "Waiting period from On hold to parts ready state": "فترة الانتظار من التعليق إلى جاهزية قطع الغيار",
  "Waiting period from quotation to parts ready state": "فترة الانتظار من عرض السعر إلى جاهزية قطع الغيار",
  "Scheduled": "مجدول",
  "Parts Ready & Rescheduled": "قطع الغيار جاهزة وأُعيدت جدولتها",
  "On hold": "معلّق",
  "Technician Jobs": "أعمال الفنيين",
  "Technician Jobs - RTAT AVG": "أعمال الفنيين - متوسط زمن الاستجابة",
  ...
  "On Hold -SP Req Hours": "ساعات التعليق بانتظار قطع الغيار",
  "Customer Need Quote Hours": "ساعات العميل بحاجة إلى عرض سعر",
...
export class PbiServiceDashboard extends Component {
  ...
  setup() {
    ...
    // Chart bodies are dynamic (t-foreach over state.items), so their DOM
    // nodes don't exist until OWL actually commits the patch — onRendered
    // fires too early (before the DOM patch is applied, only after the
    // virtual render step), so calling renderAll() there writes into
    // containers that aren't attached yet and silently no-ops. onPatched
    // fires AFTER the real DOM is updated, so every mutation to
    // state.items/state.charts (initial load, drill-down, breadcrumb nav)
    // reliably finds live containers to draw into. onPatched only fires on
    // UPDATES (not the initial mount), which is fine here — at mount time
    // state.items is still empty (load() hasn't resolved), so there'd be
    // nothing to draw anyway; the load()-triggered state mutation is
    // itself an update and gets its own onPatched call.
