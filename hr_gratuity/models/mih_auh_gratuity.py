...
class MihAuhGratuitySheet(models.Model):
    ...
    total_esob_amount = fields.Float(string="End of Service Award (EOS) Amount", compute='_compute_total_deserved_amount')
    ...
    current_last_amt = fields.Float(string="Current-Last Month Amount", invisible=True)
    ...
    def _compute_current_month_amt(self):
        for rec in self:
            ...
            rec_b = self.env['mih.auh.gratuity.sheet'].search([('id', '!=', rec.id), ('custom_late_working_day', '<', rec.custom_late_working_day),
                                                               ('custom_employee_id', '=', rec.custom_employee_id.id)], order='custom_late_working_day desc', limit=1)  # Another record
            rec_emp_b = self.env['hr.employee'].search([('id', '=', rec.custom_employee_id.id),('eos_accrued_date', '<', rec.custom_late_working_day)],  order='eos_accrued_date desc', limit=1)
    ...
    def action_lock(self):
        ...
        rec_emp_b = self.env['hr.employee'].search(
            [('id', '=', self.custom_employee_id.id), ('eos_accrued_date', '<', self.custom_late_working_day)],
        ...
        for emp in rec_emp_b:
            ...
            eos_vals = {
                'eos_amount': self.current_last_amt,
    ...
    def _compute_esob_days(self):
        for rec in self:
            if rec.custom_type == 'less_than_five_year':
                esob_days = (rec.custom_eligible_days * 21)/365
                rec.esob_days = float_round(esob_days,2)
            else:
                esob_days_f = (rec.eligible_days_f_five_years * 21)/365
    ...
    def _compute_esob_a_days(self):
        for rec in self:
            if rec.custom_type == 'less_than_five_year':
                rec.esob_a_days = float_round((rec.eligible_days_a_five_years * 21)/365,2)
            else:
                rec.esob_a_days = float_round((rec.eligible_days_a_five_years * 21)/365,2)
