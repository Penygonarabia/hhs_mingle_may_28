# -*- coding: utf-8 -*-
"""Backs every "PBI Dashboards > Promoter Dashboards" leaf. Inherits
...
which registry (PROMOTER_BOARDS, not BOARDS) they read from differ.
...
promoter_showroom/promoter_showroom_sales/sales_target directly (see
service_sql.py's promoter_showrooms/promoter_sales/sales_comparison
CTEs) — no ks_dashboard_ninja or promoter_dashboards module dependency.
...
from odoo.addons.pbi_dashboards.controllers import board_sql
from odoo.addons.pbi_dashboards.controllers.board_engine import PbiDashboardBoardEngineMixin
from .promoter_config import PROMOTER_BOARDS


class PbiPromoterDashboardController(PbiDashboardBoardEngineMixin, http.Controller):

    @http.route('/pbi_dashboards/promoter/board', type='json', auth='user')
    def promoter_board_data(self, board, dateFilter=None, customStart=None, customEnd=None, region=None):
        if not self._has_access(board):
            return {'error': 'You do not have access to this dashboard.'}
        board_cfg = PROMOTER_BOARDS.get(board)
        if not board_cfg:
    ...
    @http.route('/pbi_dashboards/promoter/chart', type='json', auth='user')
    def promoter_chart_data(self, board, item, drillPath=None, dateFilter=None, customStart=None, customEnd=None, region=None):
        if not self._has_access(board):
            return {'error': 'You do not have access to this dashboard.'}
        board_cfg = PROMOTER_BOARDS.get(board)
        if not board_cfg:
