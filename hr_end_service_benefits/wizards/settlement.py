...
class Settlement(models.TransientModel):
    ...
    amount = fields.Float(related='request_id.total_reward')
    ...
    # Settle Reward Method
    def settle_employee_reward(self):
        for record in self:
            # Check for missing partner address
            if not record.employee_id.address_id:
                raise exceptions.ValidationError(
                    _("This employee has no private address. Please add it to the employee profile."))

            # Check for payable account
            if not record.employee_id.address_id.property_account_payable_id:
                raise exceptions.ValidationError(
                    _("This employee has no payable account. Please set it in the employee's private address partner."))
            ...
            # Debit & Credit Lines
            amount = record.request_id.total_reward
            debit_account_id = record.expense_account_id.id
            credit_account_id = record.employee_id.address_id.property_account_payable_id.id

            # Debit line
            if debit_account_id and record.employee_id.address_id:
                debit_line = (0, 0, {
                    'name': name + ' debit line',
            ...
            # Payment Creation
            # self._create_payment(record, move)

    # Create Payment Helper Function
    # def _create_payment(self, record, move):
    #     if not record.settlement_journal_id.outbound_payment_method_line_ids:
    #         raise exceptions.ValidationError(_("No outbound payment method found for the selected journal."))
    #
    ...
    #         'payment_method_id': record.settlement_journal_id.outbound_payment_method_line_ids[0].id,
    ...
    #     #     self._create_payslip_payment(record)
    #
    #     # Update request status to 'paid'
    #     record.request_id.write({'account_move_id': move.id, 'payment_id': payment_id.id, 'state': 'paid'})
    #
    # # Create Payslip Payment Helper Function
    # def _create_payslip_payment(self, record):
    ...
    #     record.request_id.write({'payslip_payment_id': payslip_payment_id.id})


class Payment(models.Model):
    _inherit = 'account.payment'

    reward_id = fields.Many2one(comodel_name="hr.end.service.benefit", string="Reward")
